    ...
    <div class="panel-body">
        ...
        <div class="form-group col-md-12">
          ...
          <div class="alert alert-info">
            <strong><i class="fa fa-info-circle"></i></strong> change image where already exist or add image
          </div>
        </div>
        ...
        <div class="form-group col-md-12">
          <table class="table table-stripped" id="dynamic_table_field">
          	<?php $i=0; foreach($img as $a) { ?>
          		<?php $i++ ?>
          		<tr id="row<?= $i ?>">
          			<td><button name="remove" id="<?= $i ?>" class="btn btn-danger btn-sm btn-remove-add">X</button>
          			</td>
          			<td><input type="file" onchange="myfn(this)" name="userfile<?= $i ?>" id="event_images<?= $i ?>" data-panelid="event_images<?= $i ?>" class="form-control images_list" accept="image/gif, image/png, image/jpeg, image/pjpeg" data-change="<?= $a['path'] ?>" data-id-img="<?= $a['id_img'] ?>" />
        			</td>
...
<script type="text/javascript">
  ...
  $("input:file").change(function (){
    ...
    var bd = $(this).attr('data-id-img');
    	$('#data_del').append('<input type="text" value="'+bc+'" name="data_del[]" class="form-control"><input type="text" value="'+bd+'" name="id_img[]" class="form-control">');
  });
...
$(document).ready(function(){
        ...
        $("#add_more_img").click(function(e){
            ...
            +'</td><td id="img_preview_td"><img class="prv" id="img_preview'+i+'" /><input type="text" name="pnd" value="1"></td></tr>');
            e.preventDefault();
        });
